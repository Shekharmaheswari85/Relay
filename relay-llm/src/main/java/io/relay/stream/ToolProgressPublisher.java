...
/**
 * Routes tool-execution lifecycle events to the {@link PipelineEmitter} of the
 * session
 * that triggered the tool call.
 *
 * <p>
 * Each agent turn that opens a streaming response registers its
 * {@link PipelineEmitter}
 * via {@link #register}. Tool aspects and interceptors then call the
 * {@code emit*} methods
 * to push {@code tool_progress} and related SSE events directly to the emitter
 * without
 * requiring access to any reactive pipeline.
 *
 * <p>
 * The session ID is resolved from {@link SessionContextHolder#get()}, which is
 * set on
 * the virtual thread handling the request. No explicit session ID argument is
 * needed for
 ...
 * <p>
 * Override {@link #persistToolProgressEvent} in a subclass to write events to
 ...
 *
 * <p>
 * This class is thread-safe. The emitter map uses {@link ConcurrentHashMap}.
 */
@Component
@Slf4j
public class ToolProgressPublisher {
    ...
    /**
     * Registers a {@link PipelineEmitter} for the given session. Call once when an
     * agent
     * turn starts and a streaming HTTP response is opened.
     *
     * @param sessionId the unique session identifier; must not be {@code null}
     * @param emitter   the emitter to associate with the session; must not be
     *                  {@code null}
     */
    ...
    /**
     * Removes the {@link PipelineEmitter} for the given session. Call once the
     * agent turn
     * ends so the emitter is not retained in memory.
     *
     * @param sessionId the session identifier to deregister; a {@code null} or
     *                  unknown ID
     *                  is a no-op
     */
     ...
     *
     * <p>
     * The session ID is resolved from {@link SessionContextHolder#get()}; the event
     * is
    ...
    /**
     * Emits a {@code tool_progress} SSE event with status {@code "started"} and an
     * optional
     * summary of the tool's input arguments.
     *
     * @param toolName     the canonical tool name
     * @param message      a human-readable description of what the tool is about to
     *                     do
     * @param inputSummary a brief, non-sensitive summary of the tool input; may be
     *                     {@code null}
     */
     ...
     * @param durationMs wall-clock execution time in milliseconds; may be
     *                   {@code null}
     */
    ...
    /**
     * Emits a {@code tool_progress} SSE event with status {@code "completed"} and
     * an optional
     ...
     * @param durationMs    wall-clock execution time in milliseconds; may be
     *                      {@code null}
     * @param outputSummary a brief, non-sensitive summary of the tool result; may
     *                      be {@code null}
     */
     ...
     * @param durationMs wall-clock execution time in milliseconds before the
     *                   failure; may be
     *                   {@code null}
     */
     ...
     *
     * <p>
     * Unlike the tool-lifecycle methods, this method requires an explicit
     * {@code sessionId}
     * because it may be called before {@link SessionContextHolder} has been
     * populated.
     *
     * @param sessionId the session to target; silently ignored when {@code null},
     *                  blank, or
     *                  no emitter is registered for the ID
     * @param message   a human-readable description of what the agent is reasoning
     *                  about
     */
     ...
     *
     * <p>
     * A notice is an informational, non-critical event that the client may display
     * as a
     * soft indicator (e.g., "cache hit — skipping network call"). It is silently
     * dropped when
     ...
     * @param reason   additional context explaining why the notice was raised; may
     *                 be {@code null}
     */
    ...
    /**
     * Emits an {@code agent_handoff} SSE event notifying the client that control
     * has been
     ...
     *                  blank, or
     *                  no emitter is registered for the ID
     * @param toAgent   the name of the agent receiving control; must not be
     *                  {@code null}
     * @param reason    a brief human-readable reason for the handoff; may be
     *                  {@code null}
     */
     ...
     *
     * <p>
     * Resolves the session ID from {@link SessionContextHolder}, calls
     * {@link #persistToolProgressEvent} for optional persistence, then pushes a
     * {@code tool_progress} SSE event to the session's emitter. The emit is
     * silently dropped
     ...
     * @param durationMs    wall-clock duration in milliseconds; {@code null} when
     *                      not applicable
    ...
    /**
     * Extension point called by {@link #emit} before each event is pushed to the
     * emitter.
     * The default implementation is a no-op.
     *
     * <p>
     * Override in a subclass annotated with {@code @Component} (combined with the
     * {@code @ConditionalOnMissingBean} guard on this class) to write tool
     * execution records
     ...
     *                   {@code "failed"}
     * @param message    human-readable description
     * @param durationMs wall-clock duration in milliseconds; {@code null} when not
     *                   applicable
     */
    protected void persistToolProgressEvent(
