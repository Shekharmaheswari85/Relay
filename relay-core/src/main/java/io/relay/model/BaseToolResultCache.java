 ...
 * external API cost, and ensures deterministic replay behavior.
 *
...
public abstract class BaseToolResultCache {
     ...
     * JSON-serialized tool arguments.  Two calls to the same tool with identical
    ...
    /**
     * JSON-serialized result returned by the tool on its first (real) execution.
     * <p>
     * On a cache hit the framework deserializes this value and returns it to the
     ...
     * <p>
     * Initializes both {@code createdAt} and {@code updatedAt} to the current time.
     */
    @PrePersist
