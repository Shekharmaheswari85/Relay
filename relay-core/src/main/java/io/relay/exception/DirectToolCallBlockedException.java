...
/**
 * Thrown by {@code io.relay.guard.ToolSessionGuard#assertSessionContext(String)} when a
 ...
 * valid session context so that the framework can enforce authorization, apply the advisor
 ...
 *       {@link io.relay.session.SessionContextHolder#set(String)} before calling the
...
public class DirectToolCallBlockedException extends RuntimeException {
     ...
     * a reason other than a missing session (e.g. an expired or unauthorized session).
     *
     * @param toolName the canonical {@code @Tool(name = ...)} value of the blocked tool
